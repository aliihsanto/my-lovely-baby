...
const ProductDetail: React.FC = () => {
    ...
    // Find the product data structure from shared utility
    const productData = babyDiapersList.find(p => p.id === id);

    // If product not found, redirect to category page
    if (!productData) {
        return <Navigate to={getLocalizedPath('babyDiapers', language)} replace />;
    }

    // Get content translations for this specific ID
    const itemKey = productData.id as keyof typeof t.products.items;
    const itemContent = t.products.items[itemKey];
    ...
    return (
        <div className="min-h-screen bg-white">
            <SEO
                ...
                keywords={`${itemContent.title}, ${itemContent.weight_val}, baby diaper, bebek bezi`}
            />
            {/* Detail Hero */}
            <div className="pt-32 pb-16 bg-gradient-to-b from-brand-tealLight/30 to-white">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <Link to={getLocalizedPath('babyDiapers', language)} className="inline-flex items-center text-gray-500 hover:text-brand-teal mb-8 transition-colors font-medium">
                    ...
                    <div className="grid md:grid-cols-2 gap-12 items-center">
                        ...
                        <div className="relative">
                            <div className="aspect-square bg-white rounded-3xl shadow-xl overflow-hidden relative flex items-center justify-center p-8">
                                ...
                                {/* Product Package Mockup (Dynamic based on ID) */}
                                <img
                                    src={productData.imageJumbo}
                        ...
                        <div>
                            ...
                            <div className="flex gap-4">
                                <Link to={`${getLocalizedPath('contact', language)}?product=Baby Diaper ${itemContent.title}`} className="flex-1 bg-brand-teal text-white text-center py-4 rounded-full font-bold hover:bg-teal-600 transition-colors shadow-lg shadow-brand-teal/30">
