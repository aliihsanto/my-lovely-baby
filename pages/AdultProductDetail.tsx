...
const AdultProductDetail: React.FC = () => {
    const { t, language } = useLanguage();
    const { id } = useParams();

    const productData = adultDiapersList.find(p => p.id === id);

    if (!productData) {
        return <Navigate to={getLocalizedPath('adultDiapers', language)} replace />;
    }

    const itemKey = productData.id as keyof typeof t.products.adult_specific;
    const itemContent = (t.products.adult_specific as any)[itemKey];
    ...
    return (
        <div className="min-h-screen bg-white">
            <SEO
                ...
                keywords={`${itemContent.title}, adult diaper, hasta bezi, incontinence`}
            />
            <div className="pt-32 pb-16 bg-gradient-to-b from-blue-50 to-white">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <Link to={getLocalizedPath('adultDiapers', language)} className="inline-flex items-center text-gray-500 hover:text-blue-600 mb-8 transition-colors font-medium">
                    ...
                    <div className="grid md:grid-cols-2 gap-12 items-center">
                        <div className="relative">
                            <div className="aspect-square bg-white rounded-[2.5rem] shadow-2xl border-8 border-white overflow-hidden relative flex items-center justify-center">
                                <div className="absolute top-0 right-0 w-2/3 h-2/3 bg-blue-100 rounded-full blur-3xl"></div>
                                <img
                        ...
                        <div>
                            <span className="inline-block py-1 px-3 rounded-full bg-blue-100 text-blue-600 text-sm font-bold mb-4 tracking-wider uppercase">
                                {productData.waist}
                            </span>
                            ...
                            <div className="flex gap-4">
                                <Link to={`${getLocalizedPath('contact', language)}?product=Adult Diaper ${itemContent.title}`} className="flex-1 bg-brand-teal text-white text-center py-4 rounded-full font-bold hover:bg-teal-600 transition-colors shadow-lg shadow-brand-teal/30">
                ...
                <div className="max-w-4xl mx-auto px-4 sm:px-6">
                    ...
                    <div className="bg-white rounded-3xl shadow-xl overflow-hidden">
                        <div className="grid grid-cols-1 md:grid-cols-2 divide-y md:divide-y-0 md:divide-x divide-gray-100">
                            <div className="p-8 md:p-12 space-y-6">
                                <div>
                                    ...
                                    <div className="text-3xl font-bold text-blue-600">{itemContent.title}</div>
                                </div>
                                <div>
                        ...
                        <div className="bg-blue-50 p-6 text-center border-t border-blue-100">
                            <p className="text-blue-800 font-bold text-sm uppercase tracking-widest">{t.products.wholesale_info}</p>
                        </div>
                    </div>
                </div>
            </div>
...
export default AdultProductDetail;
